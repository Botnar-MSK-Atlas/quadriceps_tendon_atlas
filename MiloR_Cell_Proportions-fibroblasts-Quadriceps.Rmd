...
```{r}
so.quads <- readRDS("/project/tendonhca/shared/chromium/analysis/20230922_quadriceps/Quads_analysis_RDS/202419_quads_fibroblasts_annotated.RDS")
so.quads

DimPlot(so.quads, split.by = "tendon_disease")
...
myresults <- da_results$logFC
myresults <- as.factor(myresults)


quads_milo <- buildNhoodGraph(quads_milo, overlap=5)
p1 <- plotNhoodGraphDA(quads_milo, layout="UMAP", milo_res=da_results, alpha=0.1) + 
    scale_colour_gradient2(low = muted("#117733"), mid = "white", high = muted("#ff3d00"), midpoint = 0)
p1 
...
p2 <- plotNhoodGraph(quads_milo, layout="UMAP", colour_by="fibroblastcluster_id") +    scale_fill_manual(values=quads.colours) +
...
nh_graph_pl <- plotNhoodGraphDA(quads_milo, da_results,
    ...
    scale_color_gradient2(midpoint=0, low="#4b2480", mid="white", high="#eb7323", space ="Lab")
...
```{r}
da_results <- annotateNhoods(quads_milo, da_results, coldata_col = "fibroblastcluster_id")
ggplot(da_results, aes(fibroblastcluster_id_fraction)) + geom_histogram(bins=50)
...
```{r}
da_results$fibroblastcluster_id <- ifelse(da_results$fibroblastcluster_id_fraction < 0.7, "Mixed", da_results$fibroblastcluster_id)
```
...
a <- da_results %>% 
    ...
    filter(Diff == 1) %>%
    select(logFC, SpatialFDR, Diff, fibroblastcluster_id) %>% 
    head(n=50)
print(a)
...
da_results_subset <- da_results %>% filter(fibroblastcluster_id != "Mixed")

plotDAbeeswarm(da_results_subset, group.by = "fibroblastcluster_id") + 
...
ggplot(da_results, aes(x=logFC, y=-log10(SpatialFDR), colour=fibroblastcluster_id)) +
    geom_hline(yintercept=-log10(0.1), lty=2, colour='grey50') +
    geom_point(size=2) +
...
```{r}
# save the object
saveRDS(quads_milo, "/ceph/project/tendonhca/shared/chromium/analysis/20230922_quadriceps/Quads_analysis_RDS/Quads-fibroblasts-milo.RDS")

# save the results
write.table(da_results, 
            file = "/ceph/project/tendonhca/shared/chromium/analysis/20230922_quadriceps/Quads_analysis_RDS//Quads-fibroblasts-milo-DA-results.txt",
